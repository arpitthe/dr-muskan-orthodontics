...
function makeBuffer(gl: WebGL2RenderingContext, sizeOrData: ArrayBufferView, usage: number): WebGLBuffer | null {
...
function createAndSetupTexture(gl: WebGL2RenderingContext, minFilter: number, magFilter: number, wrapS: number, wrapT: number): WebGLTexture | null {
...
class InfiniteGridMenu {
  ...
  constructor(
    ...
    // Set initial camera z to be zoomed out (whole sphere visible)
    this.camera.position[2] = 3 * scale + 2.5;
    this.#init(onInit);
  }
  ...
  #init(onInit: ((menu: InfiniteGridMenu) => void) | null) {
    ...
    if (!prog) throw new Error("Could not link shader program");
  ...
  #initTexture() {
    ...
    if (!ctx) throw new Error("Could not create 2D canvas context for texture atlas");
    const cellSize = 512;

    canvas.width = this.atlasSize * cellSize;
    canvas.height = this.atlasSize * cellSize;

    // Draw solid color backgrounds or transparent cells first
    ...
    Promise.all(
      this.items.map(
        item =>
          new Promise<HTMLImageElement>(resolve => {
            ...
            img.onerror = () => {
              // Create dynamic fallback avatar (colored circle with first initial) if CORS or loading fails
              ...
              if (ctxFallback) {
                ctxFallback.fillStyle = '#c5a059'; // premium-gold
                ctxFallback.beginPath();
                ctxFallback.arc(64, 64, 64, 0, Math.PI * 2);
                ctxFallback.fill();

                ctxFallback.fillStyle = '#0b1325'; // premium-navy
                ctxFallback.font = 'bold 64px sans-serif';
                ctxFallback.textAlign = 'center';
                ctxFallback.textBaseline = 'middle';
                ctxFallback.fillText(item.title.charAt(0).toUpperCase() || 'P', 64, 64);
              }
              const fallback = new Image();
              fallback.src = canvasFallback.toDataURL();
              fallback.onload = () => resolve(fallback);
            };
            // Append cache busting parameter to bypass cached CORS issues
            img.src = item.image + (item.image.includes('?') ? '&' : '?') + 't=' + Date.now();
          })
      )
    ).then(images => {
      images.forEach((img, i) => {
        ...
        // Draw round circular clip for avatars so they display beautifully on WebGL discs
        ctx.save();
        ctx.beginPath();
  ...
  #animate(deltaTime: number) {
    ...
    if (!this.hasInteracted) {
      // Rotate the globe slowly around the Y axis on page load
      const autoRotateSpeed = 0.0006 * deltaTime;
      const rot = quat.setAxisAngle(quat.create(), [0, 1, 0], autoRotateSpeed);
  ...
  #render() {
    ...
    // Alpha blending configuration for transparent canvas
    gl.enable(gl.BLEND);
  ...
  #onControlUpdate(deltaTime: number) {
    ...
    if (this.hasInteracted) {
      ...
      // Zoom out to keep the whole globe visible initially
      cameraTargetZ += 5.5;
    }

    this.camera.position[2] += (cameraTargetZ + 0.8 - this.camera.position[2]) / damping;
    this.#updateCameraMatrix();
  }
  ...
  const handleButtonClick = () => {
    ...
    if (activeItem.link.startsWith('http')) {
      ...
      if (activeItem.link.startsWith('#')) {
        const targetId = activeItem.link.substring(1);
        const targetElement = document.getElementById(targetId);
        if (targetElement) {
          targetElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
          // Dispatch custom event to trigger testimonial card spotlight glow
          const event = new CustomEvent("highlightTestimonial", { detail: { name: activeItem.title } });
    ...
    // Reset the orbit state immediately after redirection
    setHasInteracted(false);
    setActiveItem(null);
    if (sketchRef.current) {
      sketchRef.current.hasInteracted = false;
      sketchRef.current.control.hasInteracted = false;
      sketchRef.current.control.snapTargetDirection = undefined;
    }
  };
